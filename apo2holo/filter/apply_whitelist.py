...
def apply_whitelist_build(cofactors: dict[str, list[tuple[str, float, float, float]]]) -> dict[str, list[tuple[str, float, float, float]]]:
    ...
    for key in cofactors.keys():
        if cofactors[key]["res_name"] in COFACTOR_WHITELIST or cofactors[key]["res_name"] in ACTIVE_SITE_WHITELIST:
            res[key] = cofactors[key]
        else:
            not_whitelisted[key] = cofactors[key]
    return res, not_whitelisted

def apply_active_site_whitelist(cofactors: dict[str, list[tuple[str, float, float, float]]]) -> dict[str, list[tuple[str, float, float, float]]]:
    res = {}
    
    for key in cofactors.keys():
        if key in ACTIVE_SITE_WHITELIST:
            res[key] = cofactors[key]
    return res

def apply_fes_whitelist(cofactors: dict[str, list[tuple[str, float, float, float]]]) -> dict[str, list[tuple[str, float, float, float]]]:
    res = {}
