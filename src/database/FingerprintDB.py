...
class FingerprintDB:
    def __init__(self, db_directory: Path = None):
        ...
        # models
        self.as_logistic_regression_model_path = os.path.join(
            str(db_directory), "as_logistic_regression_model.pkl"
        )
        self.fes_logistic_regression_model_path = os.path.join(
            str(db_directory), "fes_logistic_regression_model.pkl"
        )
        ...
        self.as_random_forest_model_path = os.path.join(
            str(db_directory), "as_randomforest_model.pkl"
        )
        self.fes_random_forest_model_path = os.path.join(
            str(db_directory), "fes_randomforest_model.pkl"
        )
        ...
        if os.path.exists(self.as_logistic_regression_model_path):
            self.as_logistic_regression_model = LogisticRegressionModel().load(
                self.as_logistic_regression_model_path
            )
        if os.path.exists(self.fes_logistic_regression_model_path):
            self.fes_logistic_regression_model = LogisticRegressionModel().load(
                self.fes_logistic_regression_model_path
            )
        ...
        if os.path.exists(self.as_random_forest_model_path):
            self.as_random_forest_model = CustomRandomForestClassifier().load(
                self.as_random_forest_model_path
            )
        if os.path.exists(self.fes_random_forest_model_path):
            self.fes_random_forest_model = CustomRandomForestClassifier().load(
                self.fes_random_forest_model_path
            )
    ...
    def _process_structure(
        ...
        if extend_background_samples:
            ...
            samples = np.random.normal(
                loc=center, scale=sigma, size=(AMOUNT_ARTIFICAL_SAMPLES, 3)
            )
