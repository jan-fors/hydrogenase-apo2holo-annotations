...
from src.utils.protein.apply_transformations_to_cofactors import (
...
from src.utils.geometric.calculate_geometric_centers import (
    calculate_geometric_centers,
    calculate_center,
)
from src.io.plot import plot_protein
from src.filter.apply_blacklist import apply_blacklist
from src.filter.apply_whitelist import (
    apply_whitelist,
    apply_active_site_whitelist,
    apply_fes_whitelist,
)
...
def _active_site_structure_db_search(
    structureDB: StructureDB, chain_paths: List[Path], out: Path, chain_dir: Path
...
def _arrange_cofactors_into_clusters(
    cofactor_sites: List[tuple[float, float, float]],
) -> dict:
    """ """
    # perform clustering
...
def _identify_possible_active_site_pockets(
    structure_db_path: Path, input_path: Path, out: Path, chain_dir: Path
) -> tuple[list, dict]:
    """
...
def _predict_active_site_prob_by_pockets(
...
def _identify_possible_fes_pockets(
...
def _predict_fes_prob_by_pockets(
    ...
    for key in pockets.keys():
        if search_type == "absolut":
            ...
            # search
            printl(f"Searching pocket {key} against fingerprint database...")
            hits = fes_fingerprintDB.fes_search(
                F=fingerprints, search_type=search_type, model=model
            )
...
def main(
    ...
    fingerprint_db_path: Path = FINGERPRINT_DB,
    ...
    chain_paths, active_site_pockets = _identify_possible_active_site_pockets(
