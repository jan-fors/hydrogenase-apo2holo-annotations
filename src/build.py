...
def parse_args(args):
    ...
    return input_structure_dir, output_structure_dir, structure_db_path, fingerprint_db_path, args.jobs

def build(input_structure_dir : Path, output_structure_dir : Path, structure_db_path : Path, fingerprint_db_path : Path, jobs : int):
    """
    """
    # build structure db
    #build_structure_db(input_structure_dir, output_structure_dir, structure_db_path, jobs)

    # build fingerprint db
    build_fingerprint_db(input_structure_dir, fingerprint_db_path, jobs)

def build_structure_db(raw_input_dir : Path, chain_dir : Path, structure_db_path : Path, jobs : int):
    """
    """
...
if __name__ == "__main__":
    ...
    input_structure_dir, output_structure_dir, structure_db_path, fingerprint_db_path, jobs = parse_args(args)
    build(input_structure_dir, output_structure_dir, structure_db_path, fingerprint_db_path, jobs)